 ...
 */
void compute_stats() {
	uint64_t sum_all = ((stats.accesses * MEMORY_ACCESS_TIME) + (stats.page_faults * DISK_PAGE_READ_TIME) + (stats.writebacks * DISK_PAGE_WRITE_TIME));
	stats.amat = sum_all / (stats.accesses + stats.page_faults + stats.writebacks);
}
